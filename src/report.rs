...
/// The passing status label, widened only as far as the analysis actually
    ...
    /// Sort categories with the most severe findings first: any category
    /// containing a Critical finding sorts before any Warning-only category,
    /// which sorts before any Info-only category. A category can hold findings
    /// of mixed severity — it is ranked by its most severe finding, since that
    /// is the one a reader most needs to see early. Ties within a severity
    /// band are broken alphabetically for stable output.
...
pub fn get_remediation_guidance(category: &str) -> Option<&'static str> {
    match category {
        ...
        "Function Renamed" => Some("This is a breaking change. A function was renamed which breaks existing callers. Update client integrations to use the new function name."),
        ...
        "Parameter Added" => Some("This is a breaking change. Adding parameters breaks positional/named RPC invocation. Restore the original parameter structure or update all client integrations."),
        "Parameter Removed" => Some("This is a breaking change. Removing parameters breaks positional/named RPC invocation. Restore the parameter or update all client integrations."),
        ...
        "Return Type Success Arm Changed" => Some("This is a breaking change. Update caller expectations and client SDKs to match the new success return type."),
        "Return Type Error Arm Changed" => Some("This is a breaking change. Update error handling code and client SDKs to handle the new return error type."),
