...
use stellar_xdr::curr::{
    ContractDataEntry, ScSpecTypeDef, ScSpecUdtUnionCaseV0, ScVal,
};
...
pub fn validate_scval_type(
    ...
    match type_def {
        ...
        ScSpecTypeDef::Result(res) => {
            match val {
                ScVal::Error(_) => validate_scval_type(val, &res.error_type, spec, &format!("{}.ResultErr", path)),
            ...
            match val {
                ScVal::Vec(sc_vec) => {
                    ...
                    if elements.len() != expected_types.len() {
                        return Err(format!("{}: tuple size mismatch (expected {}, got {})", path, expected_types.len(), elements.len()));
                    }
                    for (i, (elem, expected_t)) in elements.iter().zip(expected_types.iter()).enumerate() {
        ...
        ScSpecTypeDef::BytesN(b_def) => {
            match val {
                ScVal::Bytes(bytes) => {
                    if bytes.0.len() == b_def.n as usize {
                        Ok(())
                    } else {
                        Err(format!("{}: expected BytesN({}), got bytes of length {}", path, b_def.n, bytes.0.len()))
                    }
                }
        ...
        match val {
            ScVal::Map(sc_map) => {
                ...
                for field in struct_def.fields.iter() {
                    ...
                    match found_entry {
                        Some(entry) => {
                            validate_scval_type(&entry.val, &field.type_, spec, &format!("{}.{}", path, f_name))?;
                        }
                        None => {
                            if !is_option_type(&field.type_) {
                                return Err(format!("{}: missing required field '{}'", path, f_name));
                            }
                        }
                    }
                }
            ...
            ScVal::Vec(sc_vec) => {
                ...
                for (i, (elem, field)) in elements.iter().zip(struct_def.fields.iter()).enumerate() {
                    validate_scval_type(elem, &field.type_, spec, &format!("{}.{}", path, field.name))?;
                }
                Ok(())
            }
            _ => Err(format!("{}: expected struct '{}', got {:?}", path, udt_name, val)),
        }
        ...
        match val {
            ...
            ScVal::Vec(sc_vec) => {
                ...
                match &elements[0] {
                    ...
                    _ => Err(format!("{}: expected Symbol as enum variant name, got {:?}", path, elements[0])),
                }
            }
            ScVal::U32(v) => {
                if enum_def.cases.iter().any(|c| c.value == *v) {
                    ...
                    Err(format!("{}: invalid enum value {} for enum '{}'", path, v, udt_name))
                }
            }
            ScVal::I32(v) => {
            ...
            _ => Err(format!("{}: expected enum '{}', got {:?}", path, udt_name, val)),
        }
    } else if let Some(union_def) = spec.unions.get(udt_name) {
        match val {
            ScVal::Symbol(sym) => {
                let name = sym.to_string();
                let case = union_def.cases.iter().find(|c| {
                    match c {
                ...
                match case {
                    ...
                    Some(_) => Err(format!("{}: union variant '{}' expects payload values", path, name)),
            ...
            ScVal::Vec(sc_vec) => {
                ...
                match &elements[0] {
                        ...
                        match case {
                            Some(ScSpecUdtUnionCaseV0::VoidV0(_)) => {
                                if elements.len() == 1 {
                                    ...
                                    Err(format!("{}: union variant '{}' expects no payload", path, name))
                                }
                            }
                            Some(ScSpecUdtUnionCaseV0::TupleV0(t)) => {
                                ...
                                for (i, (elem, expected_t)) in elements[1..].iter().zip(payload_types.iter()).enumerate() {
                                    validate_scval_type(elem, expected_t, spec, &format!("{}.{}[{}]", path, name, i))?;
                                }
                                Ok(())
                            }
                            None => Err(format!("{}: invalid union variant '{}' for union '{}'", path, name, udt_name)),
                    ...
                    _ => Err(format!("{}: expected Symbol as union variant name, got {:?}", path, elements[0])),
                }
            }
            _ => Err(format!("{}: expected union '{}', got {:?}", path, udt_name, val)),
        }
    } else {
...
pub fn find_scval_candidates(
    ...
    if validate_scval_udt(val, udt_name, old_spec, udt_name).is_ok() {
